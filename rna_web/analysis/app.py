...
@app.get("/")
def home():
    return {"status": "success", "message": "RNA analysis server is running"}
...
async def analyze(
    ...
    # 사용자가 선택한 컬럼만 사용
    result = df[[geneColumn, log2fcColumn, pvalueColumn, padjColumn]].copy()
    ...
    # 기본적인 데이터 정리
    result = result.dropna(subset=["gene", "log2fc", "pvalue", "padj"])
    result["minusLog10Pvalue"] = -np.log10(result["pvalue"])
    ...
    volcano_data = result[["gene", "log2fc", "pvalue", "padj", "minusLog10Pvalue"]].to_dict(orient="records")
